...
class AdmissionController extends Controller
{
    ...
    public function storeFirst(Request $request)
    {
        ...
        $request->session()->put('admissions.first', $validatedData);

        // Redirect to the second page with the validated data
        return redirect('/admissions/second')->with('validatedData', $validatedData);
    }

    public function storeSecond(Request $request, Admissions $admission)
    {
        $validatedData = $request->validate([
            'gender' => 'required|string',
            'dob' => 'required|date_format:Y-m-d',
            'national_id' => 'required|string|max:255',
            'marital_sts' => 'required|string|max:255',
            'alumni_sts' => 'required|string|max:255',
        ...
        return redirect('/admissions/thank-you')->with('status', 'Your application has been submitted. Please check your email  to verify your email address.');
    }

    public function second(Course $courses)
    {
        $validatedData = session('admissions.first');
        return view('admissions.second', [
            'courses' => Course::all(),
